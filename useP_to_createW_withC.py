...
def create_W(N, P, alpha_recip, alpha_conv, alpha_div, alpha_chain):
    ...
    M_theta = math.sqrt(2)*special.erfinv(1 - (2*P))# M_theta(i,j) = theta_ij
    # matrix of thresholds.  If Z_ij > theta_ij then W_ij = 1
    print("M_theta has been created. Line 29")
    sys.stdout.flush()
    ...
    print("Now finding eigenvalues of E to calculate c, d, e.  Line 44")
    sys.stdout.flush()
    D, V = np.linalg.eig(E)
    ...
    print("Eigenvalues have been calculated.  Now checking for negatives. Line 55")
    sys.stdout.flush()
    ...
    d = AA[1,1]/np.sqrt(m)
    e = AA[0,1]/np.sqrt(m)
    print("c,d,e have been calculated. Line 72")
    sys.stdout.flush()
    ...
    print("Next creating F, G, H. Line 81")
    sys.stdout.flush()
    ...
    print("matrix H has been created. Line 98")
    sys.stdout.flush()
    ...
    print("Now creating A, B. Line 104")
    sys.stdout.flush()
    ...
    print("Discriminant calculated to find A and B. Line 123")
    sys.stdout.flush()

    Asq = np.divide(np.add(-1*coeff_b, np.sqrt(Disc)),2*coeff_a)
    A = np.sqrt(Asq)
    B = np.divide(H, np.add(A,np.transpose(A)))
    print("Initial A and B have been created.  Next making zeros along diagonals. Line 129")
    sys.stdout.flush()
